...
# =====================================================================
# 🚨 核心修复：强制清除云端 Matplotlib 的顽固字体缓存
# =====================================================================
...
# ----------------- 页面基础配置与深度美化 -----------------
st.set_page_config(page_title="投标报价智能决策看板 v6.3", layout="wide")
...
st.title("📊 投标报价智能决策看板 v6.3")
st.write("核心算法：滑动雷达扫描 + 锚点外推弹性网格 + 区间火力覆盖 + 10万次蒙特卡洛验算。")
st.markdown("---")
...
with col_base1:
    ...
    min_main_step = st.number_input("主力最小战术步距 (%) - 智能防废标底线", min_value=0.01, value=0.30, step=0.05)
...
# ----------------- 4. 蒙特卡洛矩阵引擎运算 -----------------
st.header("4. 🕹️ V6.3 海量推演与区间火力网布防")

if st.button("🚀 启动 100,000 次海量兵棋推演", use_container_width=True):
    with st.spinner("正在引爆 10万次 极端市场乱战，系统正抓取大盘极值并生成区间火力网..."):
        SIM_COUNT = 100000
        ...
        for mb in main_bids:
            ax1.axvline(x=mb, color='#d32f2f', linestyle='-', linewidth=2, alpha=0.8, label='主力中轴锚点' if mb==main_bids[0] else "")
        if len(flank_bids) > 0:
            for fb in flank_bids:
                ax1.axvline(x=fb, color='#ff9800', linestyle='--', linewidth=2, alpha=0.8, label='侧翼中轴锚点' if fb==flank_bids[0] else "")

        ax1.legend(loc='upper right')
        plt.title("📈 大盘全景雷达扫描地形图 与 多梯队区间落位锚点", fontsize=12, pad=12, fontweight='bold')
        st.pyplot(fig1)

        # ----------------- 【新增】：智能区间计算逻辑 -----------------
        # 游动半径 Buffer = 战术步距的 1/3，确保安全缝隙防串标
        buffer = max(0.01, round(min_main_step / 3, 2))
        
        main_intervals_html = ""
        for i, anchor in enumerate(main_bids):
            main_intervals_html += f"<li style='margin-bottom:6px;'>主攻 {i+1} 号：<span style='font-size:16px; font-weight:bold; color:#b71c1c; background:#ffebee; padding:2px 6px; border-radius:4px;'>【 {anchor - buffer:.2f}% ~ {anchor + buffer:.2f}% 】</span> <span style='font-size:12px; color:#777;'>(中轴锚点: {anchor:.2f}%)</span></li>"

        flank_intervals_html = ""
        if len(flank_bids) > 0:
            for i, anchor in enumerate(flank_bids):
                flank_intervals_html += f"<li style='margin-bottom:6px;'>掩护 {i+1} 号：<span style='font-size:16px; font-weight:bold; color:#e65100; background:#fff3e0; padding:2px 6px; border-radius:4px;'>【 {anchor - buffer:.2f}% ~ {anchor + buffer:.2f}% 】</span> <span style='font-size:12px; color:#777;'>(中轴锚点: {anchor:.2f}%)</span></li>"
        else:
            flank_intervals_html = "<li style='color:#777;'>无掩护部队</li>"
...
<h4 style='margin:0 0 10px 0; color:#1b5e20;'>🎯 CBO 决策层：10万次推演与区间火力网布防执行书</h4>
...
在这 10 万个高度混沌的市场乱局中，系统逐一验证了我方防线。以下推荐的<b>“报价区间”</b>，正是经历 10 万次极限压力测试后，既能最大概率守住技术标红线，又能完美规避“串标/废标”风险的最优阵型。
</p>
</div>
...
<li><b>布防防线：</b> {core_min:.2f}% ~ {core_max:.2f}% (大盘绝对峰值拦截区)</li>
<li style='margin-top:8px;'><b>建议下注区间 (防废标独立网格)：</b>
    <ul style='margin-top: 5px; list-style-type: circle;'>
        {main_intervals_html}
    </ul>
</li>
<li style='color:#555; margin-top:5px;'><b>实战微操建议：</b> 经 10 万次扫描，大盘基准价落入该防线的频次最高。请一线操作团队在上述<b>各自独立的区间内</b>自由选定最终精确报价。系统已自动在各区间之间留出防火墙缝隙，杜绝撞车风险。</li>
</ul>
{expansion_note}
...
<li style='color:#555; margin-top:5px;'><b>战术意图：</b> 市场存在极小概率的极端杀价或意外高价。以此阵型拉大步距布防于主峰两侧，是为了在 K1 抽签极差时，依然有子弹能够截获基准价，防止主阵型集体踏空。</li>
</ul>
...
"""
        ...
        st.markdown("---")
        st.subheader("🛑 图二：大盘抽签规则“红绿灯”风险响应矩阵 (抽取阵型锚点验证)")
